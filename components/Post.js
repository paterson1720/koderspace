...
function Post({ post, user, fetchPosts, key, bookmarkId }) {
  ...
  const handleDeletePost = async (id) => {
    const endpoint = bookmarkId ? `/api/bookmarks/${bookmarkId}` : `/api/posts/${id}`;
    const { error, data } = await HttpService.deleteData(endpoint);
    if (error || data.error) {
      console.log(error, data.error);
      setOpenDialog(false);
      return alert('Oops! An error happen, please try again.');
    }
    await fetchPosts();
    setOpenDialog(false);
  };
  ...
  return (
    <div key={key} className={styles.postContainer}>
      ...
      {user?._id === post?.user?._id && !bookmarkId && (
